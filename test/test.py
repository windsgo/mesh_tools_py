...
if __name__ == "__main__":
    argc = len(sys.argv)
    if (argc != 6):
        print("Usage python3 (dir/)test.py [filename] [a] [b] [c] [d]")
        exit(1)
    ...
    try:
        a = mesh_slicer(mesh_file) # use False here to disable print!
        print("mesh_slicer ctor success")
    except:
        print("mesh_slicer ctor exception")
        exit(1)

    # test save
    a.save_intersection_polylines(a_, b_, c_, d_, 
                                  '{}_intersection_{}x_{}y_{}z_{}.txt'.format(mesh_file, a_, b_, c_ , d_),
                                  ',')

    # test get face indices
    faces = a.get_original_faces()
    ...
    try:
        x : np.ndarray = a.get_intersection_polylines(a_, b_, c_, d_)
    except:
        print("get_intersection_polylines exception")
        exit(1)
    ...
    plt.ylabel("y")
    # 先保存一张二维图
    plt.title('{}_intersection_{}x_{}y_{}z_{}'.format(mesh_file, a_, b_, c_ , d_))

    plt.savefig('{}_intersection_{}x_{}y_{}z_{}.png'.format(mesh_file, a_, b_, c_ , d_), dpi=900)
    plt.show()
